...
wire sign;
wire [9:0] exponentF; //need to account for exponent overflow when adding and sign.
wire [12:0] exponentD; //need to account for exponent overflow when adding and sign.
wire [47:0] mantissaF; // 1.mantissa for both a and b, so 24 bits each, total 48 bits after multiplication
wire [105:0] mantissaD; // 1.mantissa for both a and b, so 53 bits each, total 106 bits after multiplication
...
always @(*) begin
    if(mode) begin
        ...
        if ((normalA && subnB) || (subnA && normalB)) begin //one normal, one subnormal
            ...
            if(exponentD <= -52) //too small, underflow to zero
                result = {sign, 63'b0}; 
                ...
                mantissaD >> -exponentD + 1;
                result = {sign, 11b'0, mantissaD[103:52]};
            end
        ...
        if ((normalA && subnB) || (subnA && normalB)) begin //one normal, one subnormal
            ...
            if(normalA && normalB)  //both normal
                mantissaF = {1,a[22:0]} * {1,b[22:0]};

            else if(normalA && subnB)  //a normal, b subnormal
                mantissaF = {1,a[22:0]} * {0,b[22:0]};

            else if(subnA && normalB)  //a subnormal, b normal
                mantissaF = {0,a[22:0]} * {1,b[22:0]};
